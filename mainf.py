...
IMAGE_FOLDER = "images"
SOUND_FOLDER = "sounds"  # 소리 파일 저장할 폴더 추가
DEFAULT_CONFIG_FILE = os.path.join(CONFIG_FOLDER, "profile_1.json")
DEFAULT_SOUND_URL = "https://cdn.dpvm.xyz/sound.mp3"  # 기본 소리 URL
...
class SoundSettings:
    ...
    def from_dict(data):
        return SoundSettings(
            sound_path=data.get('sound_path', DEFAULT_SOUND_FILE),
...
# 프로필 클래스 - 소리 설정 추가
class Profile:
    def __init__(self, name="프로필 1", items=None, webhook=None, mcrcon=None, display=None, sound=None):
        self.name = name
        ...
        self.sound = sound if sound else SoundSettings()  # 소리 설정 추가
    ...
    def to_dict(self):
        return {
            ...
            'sound': self.sound.to_dict(),  # 소리 설정 저장
    ...
    def from_dict(data):
        ...
        # 소리 설정 로드
        if 'sound' in data:
            profile.sound = SoundSettings.from_dict(data['sound'])
...
class SoundSettingsTab(QWidget):
    def __init__(self, sound_settings=None):
        super().__init__()
        self.sound_settings = sound_settings or SoundSettings()
        
        # 기본 사운드 파일 다운로드 확인
        if not os.path.exists(DEFAULT_SOUND_FILE):
            self.download_default_sound()
        ...
        # 소리 활성화 체크박스
        self.enabled_check = QCheckBox("룰렛 소리 활성화")
        self.enabled_check.setChecked(self.sound_settings.enabled)
        form_layout.addRow("", self.enabled_check)
        
        # 틱 소리 활성화 체크박스 추가
        self.tick_enabled_check = QCheckBox("룰렛 회전 틱 소리 활성화")
        self.tick_enabled_check.setChecked(self.sound_settings.tick_enabled)
        self.tick_enabled_check.setToolTip("룰렛 회전 중 속도에 맞춰 '통통통통' 소리가 재생됩니다")
        form_layout.addRow("", self.tick_enabled_check)
        
        # 소리 파일 경로 설정
        sound_layout = QHBoxLayout()
        self.sound_path_edit = QLineEdit(self.sound_settings.sound_path)
        ...
        sound_layout.addWidget(self.browse_button)
        
        form_layout.addRow("소리 파일:", sound_layout)
        
        # 틱 소리 파일 경로 설정
        tick_sound_layout = QHBoxLayout()
        self.tick_sound_path_edit = QLineEdit(self.sound_settings.tick_sound_path)
        self.tick_sound_path_edit.setReadOnly(True)
        tick_sound_layout.addWidget(self.tick_sound_path_edit)
        
        self.browse_tick_button = QPushButton("찾아보기...")
        self.browse_tick_button.clicked.connect(self.browse_tick_sound)
        tick_sound_layout.addWidget(self.browse_tick_button)
        
        form_layout.addRow("틱 소리 파일:", tick_sound_layout)
        ...
        self.reset_sound_button.clicked.connect(self.reset_to_default_sound)
        test_layout.addWidget(self.reset_sound_button)
        ...
        # 도움말 추가
        help_label = QLabel(
            "MP3, WAV 등의 오디오 파일을 선택할 수 있습니다.\n"
            "기본 소리는 자동으로 다운로드됩니다.\n"
            "틱 소리는 룰렛이 돌아갈 때 항목 이동에 맞춰 재생됩니다.")
        help_label.setWordWrap(True)
        layout.addWidget(help_label)
    ...
    def download_default_sound(self):
        """기본 소리 파일을 다운로드"""
        try:
            print(f"기본 소리 파일 다운로드 중: {DEFAULT_SOUND_URL}")
            response = requests.get(DEFAULT_SOUND_URL, stream=True)
            
            if response.status_code == 200:
                with open(DEFAULT_SOUND_FILE, 'wb') as f:
                    for chunk in response.iter_content(chunk_size=8192):
                        if chunk:
                            f.write(chunk)
                print(f"기본 소리 파일이 다운로드되었습니다: {DEFAULT_SOUND_FILE}")
                return True
            else:
                print(f"소리 파일 다운로드 실패: {response.status_code}")
                self.create_default_tick_sound()
                return False
    ...
    def download_tick_sound(self):
        """기본 틱 소리 파일을 다운로드 또는 생성"""
        try:
            # 기본 틱 소리가 없으면 생성 시도
            print(f"기본 틱 소리 파일 다운로드 중: {DEFAULT_TICK_SOUND_URL}")
            
            try:
                response = requests.get(DEFAULT_TICK_SOUND_URL, stream=True)
                
                if response.status_code == 200:
                    with open(DEFAULT_TICK_SOUND_FILE, 'wb') as f:
                        for chunk in response.iter_content(chunk_size=8192):
                            if chunk:
                                f.write(chunk)
                    print(f"기본 틱 소리 파일이 다운로드되었습니다: {DEFAULT_TICK_SOUND_FILE}")
                    return True
                else:
                    print("틱 소리 다운로드 실패, 기본 파일 생성을 시도합니다.")
                    return self.create_default_tick_sound()
            except Exception as e:
                print(f"틱 소리 다운로드 오류: {e}")
                return self.create_default_tick_sound()
    ...
    def create_default_tick_sound(self):
        ...
        try:
            # 소리 파일을 생성하는 대신 이미 있는 소리 파일 복사
            # 실제로는 wav 파일을 생성하는 라이브러리를 사용하는 것이 좋지만, 여기서는 간단한 구현 제공
            if os.path.exists(DEFAULT_SOUND_FILE):
                import shutil
                shutil.copy2(DEFAULT_SOUND_FILE, DEFAULT_TICK_SOUND_FILE)
                print(f"기본 소리 파일을 틱 소리로 복사했습니다: {DEFAULT_TICK_SOUND_FILE}")
                return True
                
            # 기본 틱 소리를 생성하는 로직 (파이썬 wave, numpy 등 라이브러리 필요)
            # 생략 - 적절한 라이브러리로 구현 필요
                
            print("틱 소리 파일 생성 실패")
            return False
        except Exception as e:
            print(f"틱 소리 파일 생성 오류: {e}")
            return False
    
    def browse_sound(self):
        file_name, _ = QFileDialog.getOpenFileName(
            self, "소리 파일 선택", "", "소리 파일 (*.mp3 *.wav *.ogg *.m4a)"
        )
        
        if file_name:
            self.sound_path_edit.setText(file_name)
    
    def browse_tick_sound(self):
        file_name, _ = QFileDialog.getOpenFileName(
            self, "틱 소리 파일 선택", "", "소리 파일 (*.mp3 *.wav *.ogg *.m4a)"
        )
        
        if file_name:
            self.tick_sound_path_edit.setText(file_name)
    
    def test_sound(self):
        sound_path = self.sound_path_edit.text()
        
        if not os.path.exists(sound_path):
            QMessageBox.warning(self, "소리 테스트", f"파일이 존재하지 않습니다: {sound_path}")
            return
        ...
        try:
            ...
            print(f"소리 테스트: {sound_path} (볼륨: {volume}%)")
            
        except Exception as e:
            QMessageBox.critical(self, "소리 테스트", f"소리 재생 중 오류 발생: {e}")
    
    def test_tick_sound(self):
        sound_path = self.tick_sound_path_edit.text()
        
        if not os.path.exists(sound_path):
            QMessageBox.warning(self, "틱 소리 테스트", f"파일이 존재하지 않습니다: {sound_path}")
            return
        ...
        try:
            ...
            # 볼륨 설정 (0-100 범위를 0.0-1.0으로 변환)
            volume = self.volume_slider.value()
            self.tick_player.setVolume(volume)
            
            # 소리 재생
            self.tick_player.play()
            
            print(f"틱 소리 테스트: {sound_path} (볼륨: {volume}%)")
    ...
    def save_settings(self):
        self.sound_settings.sound_path = self.sound_path_edit.text()
        self.sound_settings.enabled = self.enabled_check.isChecked()
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None, profile=None):
        ...
        # 소리 설정 탭 추가
        sound_tab = SoundSettingsTab(self.profile.sound)
        ...
        tabs.addTab(sound_tab, "소리 설정")  # 소리 탭 추가
        ...
        self.sound_tab = sound_tab  # 소리 탭 참조 저장
    ...
    def accept(self):
        ...
        self.profile.sound = self.sound_tab.save_settings()  # 소리 설정 저장
        super().accept()
...
class RouletteApp(QMainWindow):
    def __init__(self):
        ...
        self.signals.play_tick_sound.connect(self.play_tick_sound)  # 틱 소리 신호 연결
    ...
    def check_default_sound(self):
        ...
        try:
            # 기본 소리 파일 확인
            if not os.path.exists(DEFAULT_SOUND_FILE):
                print("기본 소리 파일이 없습니다. 다운로드를 시도합니다.")
                self.download_default_sound()
    ...
    def play_roulette_sound(self):
        ...
        try:
            ...
            if not sound_settings.enabled:
                print("소리 기능이 비활성화되어 있습니다.")
                return False
            
            # 소리 파일 경로 확인
            sound_path = sound_settings.sound_path
            if not os.path.exists(sound_path):
                print(f"소리 파일이 존재하지 않습니다: {sound_path}")
                return False
            ...
            # 소리 로드 및 볼륨 설정
            self.sound_player.setMedia(QMediaContent(QUrl.fromLocalFile(sound_path)))
            self.sound_player.setVolume(sound_settings.volume)
            
            # 소리 재생
            self.sound_player.play()
            
            print(f"룰렛 소리 재생 시작: {sound_path} (볼륨: {sound_settings.volume}%)")
            return True
    ...
    def play_tick_sound(self):
        ...
        try:
            # 소리 설정 확인
            sound_settings = self.current_profile.sound
            if not sound_settings.enabled or not sound_settings.tick_enabled:
                return False
            
            # 소리 파일 경로 확인
            tick_sound_path = sound_settings.tick_sound_path
            if not os.path.exists(tick_sound_path):
                print(f"틱 소리 파일이 존재하지 않습니다: {tick_sound_path}")
                return False
            
            # 현재 시간
            current_time = time.time()
            
            # 마지막 틱 소리 이후 너무 짧은 시간이 지나지 않았다면 재생하지 않음
            # (소리 중복 방지)
            if current_time - self.last_tick_time < self.tick_interval * 0.5:
                return False
            ...
            # 소리 로드 및 볼륨 설정
            self.tick_player.setMedia(QMediaContent(QUrl.fromLocalFile(tick_sound_path)))
            self.tick_player.setVolume(sound_settings.volume)
            
            # 소리 재생
            self.tick_player.play()
            
            # 디버깅용 로그는 출력 부담이 크므로 비활성화
            # print(f"틱 소리 재생 (간격: {self.tick_interval:.2f}초)")
            return True
    ...
    def spin_roulette(self):
        ...
        # 룰렛 소리 재생
        self.play_roulette_sound()
        
        # 틱 소리 관련 변수 초기화
        self.last_tick_time = time.time()  # 마지막 틱 소리 시간 초기화
        self.tick_interval = 0.1  # 초기 틱 소리 간격
    ...
    def animate_roulette(self):
        ...
        try:
            ...
            while time.time() - start_time < rotation_time:
                ...
                # 틱 소리 재생 신호 발생
                self.signals.play_tick_sound.emit()
                
                # 속도 점점 느려지게
                progress = (time.time() - start_time) / rotation_time
                interval = 0.1 + progress * 0.3  # 0.1초에서 시작해서 0.4초까지 늘어남
